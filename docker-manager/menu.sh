...
show_menu() {    echo -e "${WHITE}ENVIRONNEMENTS COMPLETS (avec build):${NC}"
    ...
    echo -e "${ORANGE}  3)${NC} 🧪 Lancer TOUT en TESTS (environnement isolé + build)"
    echo ""
    echo -e "${WHITE}DÉMARRAGE RAPIDE (containers existants):${NC}"
    echo -e "${CYAN}  4)${NC} ⚡ Démarrage RAPIDE Développement (sans rebuild)"
    echo -e "${CYAN}  5)${NC} ⚡ Démarrage RAPIDE Production (sans rebuild)"
    echo -e "${CYAN}  6)${NC} ⚡ Démarrage RAPIDE Tests (sans rebuild)"
    echo ""
    echo -e "${WHITE}GESTION DES SERVICES:${NC}"
    echo -e "${CYAN}  7)${NC} 🔧 Démarrer un service spécifique"
    echo -e "${CYAN}  8)${NC} ⏹️  Arrêter un service spécifique"
    echo -e "${CYAN}  9)${NC} 🔄 Redémarrer un service spécifique"
    echo -e "${CYAN} 10)${NC} 🏗️  Builder/Rebuilder un service"
    echo ""
    echo -e "${WHITE}MONITORING & LOGS:${NC}"
    echo -e "${YELLOW} 11)${NC} 📊 Voir l'état des containers"
    echo -e "${YELLOW} 12)${NC} 📝 Voir les logs d'un service"
    echo -e "${YELLOW} 13)${NC} 📈 Suivre les logs en temps réel"
    echo -e "${YELLOW} 14)${NC} 🖥️  Ouvrir un shell dans un container"
    echo ""
    echo -e "${WHITE}MAINTENANCE:${NC}"
    echo -e "${RED} 15)${NC} 🛑 Arrêter TOUS les services"
    echo -e "${RED} 16)${NC} 🧹 Options de nettoyage (soft/complet)"
    echo -e "${RED} 17)${NC} 🔄 Restart complet (stop + build + start)"
    echo -e "${RED} 24)${NC} 💥 SUPPRESSION TOTALE Docker (DANGER !)"
    echo ""
    echo -e "${WHITE}UTILITAIRES:${NC}"
    echo -e "${BLUE} 18)${NC} 💾 Backup de la base de données"
    echo -e "${BLUE} 19)${NC} 📦 Voir l'utilisation des ressources"
    echo -e "${BLUE} 20)${NC} 🌐 Ouvrir les URLs de l'application"
    echo -e "${BLUE} 21)${NC} 🔍 Diagnostic des services (debug)"
    echo -e "${GREEN} 22)${NC} 🧪 Lancer les tests automatisés"
    echo -e "${MAGENTA} 23)${NC} 📝 Créer des données de test (reset rapide DB)"
    echo ""
