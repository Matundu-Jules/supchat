...
start_development() {
    ...
    docker compose -f docker-compose.development.yml --env-file .env.dev build --no-cache
    
    echo -e "\n${BLUE}Démarrage des services...${NC}"
    docker compose -f docker-compose.development.yml --env-file .env.dev up -d --build
...
create_manual_test_data() {
    ...
    if ! docker-compose -f docker-compose.development.yml ps api | grep -q "Up"; then
    ...
    if docker-compose -f docker-compose.development.yml exec api node create-test-users.js; then
